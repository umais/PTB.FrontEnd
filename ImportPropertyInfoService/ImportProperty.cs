...
namespace ImportPropertyInfoService
{
    public partial class ImportProperty : ServiceBase
    {
                ...
                foreach (DataRow item in dt.Rows)
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            proptbl.SalesPrice = GetValue(dt, item, "SalesPrice");
        ...
        private void AddToFallOutCSVFile(string logdata)
        {
            if (File.Exists(ConfigurationManager.AppSettings["FallOutFile"]))
            {
                string[] content = { logdata };
                File.AppendAllLines(ConfigurationManager.AppSettings["FallOutFile"], content);
            }
            else
            {
                string[] content = { "File Name, Lot, Section, Block, Date", logdata };
                File.AppendAllLines(ConfigurationManager.AppSettings["FallOutFile"], content);
            }
        }
    }
